...
NSInteger const AllCount = 999;//初始item数量
...
{
    BKCycleScrollCollectionViewCell * currentCell = (BKCycleScrollCollectionViewCell*)cell;
    ...
    if ([self.delegate respondsToSelector:@selector(cycleScrollView:customDisplayCellStyleAtIndex:displayCell:)]) {
        [[currentCell subviews] makeObjectsPerformSelector:@selector(removeFromSuperview)];
        [self.delegate cycleScrollView:self customDisplayCellStyleAtIndex:selectIndex displayCell:currentCell];
        return;
    }
    
    currentCell.radius = self.radius;
    
    NSObject * obj = self.displayDataArr[selectIndex];
    if ([obj isKindOfClass:[NSString class]]) {
        NSURL * imageUrl = [NSURL URLWithString:(NSString*)obj];
        [currentCell.displayImageView sd_setImageWithURL:imageUrl placeholderImage:self.placeholderImage];
    }else if ([obj isKindOfClass:[UIImage class]]) {
        currentCell.displayImageView.image = (UIImage*)obj;
    }else if ([obj isKindOfClass:[NSData class]]) {
        FLAnimatedImage * image = [FLAnimatedImage animatedImageWithGIFData:(NSData*)obj];
        if (!image) {
            currentCell.displayImageView.image = [UIImage imageWithData:(NSData*)obj];
        }else{
            currentCell.displayImageView.animatedImage = image;
        }
    }else{
        currentCell.displayImageView.image = nil;
    }
}
...
-(NSInteger)getDisplayIndexWithTargetIndexPath:(NSIndexPath*)indexPath
{
    ...
    if (selectIndex < 0) {
        selectIndex = [self.displayDataArr count] - 1;
    }else if (selectIndex > [self.displayDataArr count] - 1) {
        selectIndex = 0;
    }
